...
function show_shenhe_box($she_box_name) {
   ...
   // $role_tmp= get_role_access($mname,1);//需要重新读
    ...
    foreach ($she_box_name as $bname => $btitle) {
        ...
        if(($oname=='baocun') || ($oname=='shenhe') || ($oname=='next')){
          $oname='update';$oname1='create';
        }
...
function getUrl($url,$ptitle,$title,$style='',$imgclass=''){
 $s1='<a class="btn" href="'.$url.'"  title="'.$ptitle.'" ';
 return $s1.$style.'>'.$imgclass.$title.'</a>';
}
...
function show_command($command,$url="",$title="",$style='',$is_s2='') {
   ...
   // $role_tmp= get_role_access($mname);
    if ($command == '添加') {
       $s1=getUrl($url,$title,$title,'','<i class="fa fa-plus"></i>');
       $s2='create';
    } else if ($command == '修改') {
        $s1=getUrl($url,$command,$title,$style);
        $s2='update';
    } else if ($command == '批删除') {
        ...
        $s1=getUrl($url,$title,$title,$style);
        $s2=$is_s2;
    }
...
function hsYii_getHeadUpdateFrom($thisv,$title){
   return BaseLib::model()->showHeadUpdate($thisv,$title);
}

function hsYii_showTableInput($form,$model,$inputCmd){
   echo BaseLib::model()->tableInput($form,$model,$inputCmd);
}

function hsYii_showUpdateBottom($thisv,$model,$comstr){
   echo BaseLib::model()->updateBottom($thisv,$model,$comstr);
}
