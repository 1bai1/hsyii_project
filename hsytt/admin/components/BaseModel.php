 ...
 */
 public  function toTdstr($s2,$rows='',$show_type='') {
     if((indexof($s2,'uploads/temp')>=0) || ($show_type=='pic')){
       $icon=BasePath::model()->fileIcon($s2);
         $s2='<img src="'. $icon.'" height="60px" width="60px">';
       }
      if($show_type=='YN'){
         $s2=(empty($s2)) ?'否' : '是';
       } 
...
//从TMP对象设置值a:b   $this->a=$tmp->b;
  ...
  public  function setFromArray($tmp,$fieldstr){
    ...
    public function checkafterFind(){//index
       return (1==1);
    }
    public function getTempLable($pname) {
      return get_session($pname);;
    }
