...
public function addPath($str1){
 ...
 public function addlocalPath($str1){
 
    if($str1)
     if(!substr_count($str1,'http')){
  
        $str1=$this->savePath().$str1;
       }
...
public function fileIcon($flie) {
   $s1=substr($flie,-3,3);
   $istr = ".csv,.txt,.rar,.zip,.doc,.docx,.xls,.xlsx,.mp3,.mp4"; 
   $s2=$this->addPath('img/'.$s1.'_icon.png');
   $rs=$flie;
   if(stristr($istr,$s1)) $rs=$s2;
    return $rs;
  }
